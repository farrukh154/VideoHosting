...
<div class="container">
    <p  style="font-size:40px; padding: 20px 0;" class="mt-4">Смотреть позже</p>
    <div class="row"> 
        @foreach ($videos as $entry)
            <div class="col-md-4 mb-4">
                <div class="video">
                    <iframe style="border:2px solid; border-radius:20px" width="100%" height="255" src="https://www.youtube.com/embed/{{ $entry->video_id }}" frameborder="0" allowfullscreen></iframe>
                    <h5 class="pt-2 video_title" style="color: white; text-align:center;">{{ $entry->video_title }}</h5>
                    <form method="POST" action="{{ route('watch_later.remove', $entry->video_id) }}">
                        @csrf
                        @method('DELETE')
                        <button style=" border:2px solid #fff; padding: 5px 20px; margin-left: 30%;" class="btn btn-default plus btn-sm mt-2">Удалить<i  style="color: white; margin-left: 12px;" class="fa fa-close"></i></button>
                    </form>
                </div>
            </div>
        @endforeach
